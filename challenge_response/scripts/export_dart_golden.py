...
def sequence_cases():
    ...
    cases = [
        ...
        # --- 연속 세션 (2026-09-18) ---
        # 판정을 멈춘 구간(결과 표시·준비)에서도 손이 계속 잡혀야 한다.
        # 이 구간이 비면 "Challenge는 본인 손, 인증은 영상 재생"을 막지 못한다.
        {"name": "hand_lost_during_result_hold_breaks_session",
         "actions": ["OPEN_PALM", "FIST", "MOVE_RIGHT"],
         "config": {"timing": {"step_result_hold_ms": 1500}},
         "script": [{"kind": "shape", "label": "OPEN_PALM", "frames": hold},
                    {"kind": "lost", "frames": 40}]},
        {"name": "hand_lost_during_prepare_breaks_session",
         "actions": ["OPEN_PALM", "FIST", "MOVE_RIGHT"],
         "config": {"timing": {"step_result_hold_ms": 0, "step_prepare_ms": 1500}},
         "script": [{"kind": "shape", "label": "OPEN_PALM", "frames": hold},
                    {"kind": "lost", "frames": 40}]},
        {"name": "brief_dropout_during_hold_is_tolerated",
         "actions": ["OPEN_PALM", "FIST", "MOVE_RIGHT"],
         "config": {"timing": {"step_result_hold_ms": 1500}},
         "script": [{"kind": "shape", "label": "OPEN_PALM", "frames": hold},
                    {"kind": "lost", "frames": 3},
                    {"kind": "shape", "label": "FIST", "frames": 60}]},
