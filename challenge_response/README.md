...
| `escape_frames` | 10 | 손 모양 유지 중 오검출 구간 p95=8.4프레임 +1 |
...
영상의 손 모양 4종에서 라벨이 정답과 다른 구간을 세면 14개가 나오고,
p95=8.4프레임 → `escape_frames=10`이다.
...
### 5.7 이탈 관문에 남은 구멍 두 가지

**(1) 1단계에는 관문이 없다.** 이전 단계가 없으므로 `escape_from`을 정할 수 없다.
Challenge 시작 시점에 이미 1단계 손 모양을 하고 있으면 그대로 통과된다. 막으려면
`WAIT_HAND`에서 처음 손이 잡힌 시점의 모양을 `escape_from`으로 삼아야 하는데,
그러면 모든 사용자가 매번 시작 전에 손을 한 번 바꿔야 한다. 지금은 요청받은
범위(단계 전환)만 구현했다.

**(2) 잡음이 관문을 열 수 있다.** `escape_frames=10`인데 파일럿에서 관측된 오검출
구간 최대치는 12프레임이다(`P05_TWO_FINGERS_near`에서 1건). 드물게 손을 가만히
둬도 관문이 열릴 수 있다. 04가 실행 시 이 경고를 출력한다.

p95 대신 최대치를 쓰면(`escape_frames=13`) 관측된 잡음으로는 열리지 않는다.
다른 임계값과 같은 규칙(`negative_percentile`)을 쓰려고 p95를 택했지만, 보안
관문이라는 성격상 최대치가 더 맞을 수 있다. 표본이 14개뿐이라 어느 쪽도 강한
근거는 아니다. `derivation_policy.json`의 `negative_percentile`을 바꾸면 다시
도출된다.
