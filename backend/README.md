...
- `tracking.frameStale*`는 앱이 '손 없음'으로 볼 프레임 공백을 정한다. 최근 프레임
  간격의 중앙값 × factor로 유도하고 상·하한으로 묶는다. 고정 100ms를 쓰다가 14fps
  실기기에서 윈도우가 계속 비워졌다. **측정값이 아니라 정책값**이고, 도출 대상이
  아니라서 `import_challenge_config.py`는 이 세 값을 기존 값 그대로 이어받는다.
