...
ACTION 1     perActionTimeoutMs와 totalTimeoutMs가 흐른다
   ↓ 통과
PASS 1.5초   체크 아이콘 + "PASS". 테두리 = 초록. **시계 멈춤**
   ↓ timing.stepResultHoldMs
준비 1.5초   "다음: 주먹 쥐기" + 요청 동작 그림. **시계 멈춤**
...
동작 확인이 끝나면 사용자는 인증이 다 끝났다고 생각하고 손을 내린다. 실제로는
그때부터 촬영이 시작되고, 손을 내리면 세션이 끊긴다(`SESSION_BROKEN`). 배너는
세션 내내 떠 있지만 **단계가 바뀌는 순간**을 짚는 문장이 없었다. 그래서 이 순간의
문구를 따로 둔다 (`lib/models/challenge_messages.dart`).

| 시점 | 문구 |
|---|---|
| 손을 들기 전 | "손을 원 안에 올리면 시작합니다" |
| 동작 확인 중 | "인증이 끝날 때까지 손을 화면 안에 유지해주세요" |
| 마지막 단계 PASS | "손을 그대로 두세요 · 이어서 촬영합니다" (다른 단계는 "N단계 완료") |
| 촬영·전송 중 배너 | "손을 내리지 마세요. 내리면 동작 확인부터 다시 합니다" |
| 촬영 중 원 아래 | "손을 그대로 둔 채 수어 암호를 수행하세요" |

촬영 중 배너는 **손을 내리면 어떻게 되는지**까지 말한다. 그걸 알아야 사람이
손을 내리지 않는다. 판정 규칙은 그대로이고 문구만 바꿨다.

### 색 규칙: "아직"과 "통과"를 색으로 가른다

초록·민트·파랑은 전부 "됐다"로 읽힌다. 진행 중인 것에 이 색을 쓰면 아직 판정
중인데 인증된 것처럼 보이고, 정작 통과했을 때 보여줄 신호가 남지 않는다.
그래서 **초록은 PASS 순간에만** 쓴다.

| 색 | 토큰 | 쓰는 곳 |
|---|---|---|
| 회색 | `textSecondary` | 원 테두리 평소 상태. 테두리는 손을 놓을 자리일 뿐이고, 진행 상황은 시간 바와 아크가 보여준다 |
| 초록 | `success` | **PASS 배지와 그 순간의 원 테두리만** |
| 빨강 | `danger` | FAIL 배지와 원 테두리, 실패 문구, 남은 시간 30% 미만 |
| 민트 | `ring` | **통과한** 단계의 손 그림과 번호 |
| 보라 | `progress` | 아직 남은 단계와 지금 단계의 손 그림·번호, 촬영·준비 진행률 아크, "N단계 완료", "인증 요청을 보냈습니다" |
| 호박색 | `timeLeft` | 남은 시간 바, 손 유지 안내 배너 |

- **원 테두리 색은 결과가 났을 때만 바뀐다.** 평소에 색이 바뀌지 않아야 결과가
  났을 때 그 변화가 눈에 띈다.
- **보라는 진행도다. 판정 결과가 아니다.** 차오르는 아크를 초록으로 두면
  "정답"으로 읽힌다.
- **호박색은 "지금 신경 써야 할 것"이다.** 민트로 두면 "잘 되고 있다"로 읽히고,
  빨강으로 두면 이미 잘못된 줄 안다. 남은 시간 바도 처음에는 민트였다가 초록으로
  읽혀 파랑으로 바꿨는데, 파랑도 같은 문제가 있어 호박색으로 바꿨다.
- 단계 그림만 봐도 몇 단계가 남았는지 셀 수 있다. 민트는 끝난 단계, 보라는 남은 단계다
  (`ChallengeGuide.accent`).

화면에서 새 색을 즉석으로 만들지 않는다. 토큰은 `lib/core/theme.dart`의 `AppColors`에 있다.
...
challenge     손 대기 → 동작 3개 판정. 원 테두리 = 평소 회색, 결과가 났을 때만 초록/빨강
   ↓ 전 단계 통과 (카메라를 놓지 않는다)
recording     서버가 정한 시간(기본 4000ms) 수집. 테두리 위 보라 진행률 아크
   ↓ 수집 종료
...
SPEC 8.2가 요구하는 거울 모드는 **플러그인이 이미 만들어 준다.**
`camera_android_camerax` 0.7.4+6은 전면 카메라 프리뷰를 반전해서 그린다.
그래서 `buildPreview()`에서는 **다시 뒤집지 않는다.** 한 번 더 뒤집으면 반전이
상쇄되어 거울이 아니게 된다.

처음에는 플러그인이 반전하지 않는다고 보고 `buildPreview()`에서 직접 반전했다.
그 판단이 틀렸다는 것을 확인하고 지금처럼 바꿨다(위 1번의 ⚠️). 오버레이는
프리뷰에 맞춰 `mirror: true`다. 서버로 보내는 좌표는 여전히 반전하지 않은 원본이다.
